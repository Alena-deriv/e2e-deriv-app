...
describe('QATEST-2542 Place a Buy Order with same currency ads - Fixed rate', () => {
  before(() => {
    cy.clearAllSessionStorage()
  })
  beforeEach(() => {
    if (isSellAdUser == true) {
      loginWithNewUser('p2pFixedRatePKBuyOrder1', false)
    } else {
      loginWithNewUser('p2pFixedRatePKBuyOrder2', true)
    }
  ...
  it('Should be able to place an order for advert and verify all fields and messages for fixed rate.', () => {
    cy.c_navigateToP2P()
    cy.findByText('My profile').should('be.visible').click()
    cy.findByText('Available Deriv P2P balance').should('be.visible')
    cy.c_getProfileName().then((name) => {
  ...
  it("Should be able to confirm sell order from verification link, give rating to buyer and then confirm seller's balance.", () => {
    ...
    cy.c_confirmOrder(
      nicknameAndAmount,
      'buy',
      Cypress.env('credentials').test.p2pFixedRatePKBuyOrder1.ID
    )
